...
ui <- fluidPage(theme = light_theme,
  navbarPage(
    ...
    title = span(icon("youtube", style = "color: #D80808"),
                 "YouTube Trendify", style = 'font-size: 45px'),
          ...
          card(
            ...
            card_body(
              selectInput(
                inputId = "barplotcat",
                label = "Category:",
                choices = c("Music", "Education", "Gaming", "Sports","Television","Entertainment","Education","People & Blogs","Science & Technology","News & Politics","Film & Animation","Autos & Vehicles","Pets & Animals"),
                selected = "Music"
              ),
    ...
    footer = tags$div(
      ...
      p("2024 DEC © Theja Suryachar P J , AIET , Manglore")
    )
  )
)
